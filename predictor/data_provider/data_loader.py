    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            ...
            data_stamp = time_features(pd.to_datetime(
                df_stamp[self.date_header].values), freq=self.freq)
...
class Dataset_Pred(Dataset):
    ...
    def __read_data__(self):
        ...
        tmp_stamp[self.date_header] = pd.to_datetime(
            tmp_stamp[self.date_header])
        pred_dates = pd.date_range(
            tmp_stamp[self.date_header].values[-1], periods=self.pred_len + 1, freq=self.freq)
